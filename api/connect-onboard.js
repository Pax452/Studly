...
export default async function handler(req, res) {
  ...
  try {
    const { tutorId, tutorEmail, country = 'GB' } = req.body;
    if (!tutorId || !tutorEmail) return res.status(400).json({ error: 'Missing tutor details' });

    const supported = ['GB','US','AU','CA','IE','FR','DE','NL','SE','NO','DK','FI','CH','AT','BE','ES','IT','PT','NZ'];
    const accountCountry = supported.includes(country.toUpperCase()) ? country.toUpperCase() : 'GB';
    ...
    console.error('[Studly] Connect error:', err.message);
